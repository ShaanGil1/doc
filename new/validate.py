...
document can look perfect everywhere except the one place it matters.
    ...
    python3 validate_docx.py out.docx
...
ORDER = {
    ...
    "lvl": (
        ...
        "legacy lvlJc pPr rPr").split(),
    "abstractNum": (
        "nsid multiLevelType tmpl name styleLink numStyleLink lvl").split(),
    ...
    "numbering": (
        "numPicBullet abstractNum num numIdMacAtCleanup").split(),
}
...
def check_order(element, sequence_name, path, problems):
    ...
    for child in element:
        ...
        if rank < last_rank:
            problems.append(
                "%s: <w:%s> appears after <w:%s>; schema requires it before"
                % (path, name, last_name))
    ...
    for index, paragraph in enumerate(root.iter(q("p"))):
        ...
        if "pPr" in names and names[0] != "pPr":
            problems.append(
                ...
                "(found <w:%s> before it)"
                % (part, index, names.index("pPr"), names[0]))
        ppr = paragraph.find(q("pPr"))
        if ppr is not None:
            check_order(ppr, "pPr", "%s p[%d]/pPr" % (part, index), problems)
            rpr = ppr.find(q("rPr"))
            if rpr is not None:
                check_order(rpr, "rPr", "%s p[%d]/pPr/rPr" % (part, index),
                            problems)
        for run in paragraph.iter(q("r")):
            rpr = run.find(q("rPr"))
            if rpr is not None:
                check_order(rpr, "rPr", "%s p[%d]/r/rPr" % (part, index),
                            problems)
        ...
        for run in paragraph.iter(q("r")):
            ...
            for kind in fld:
                if kind == "begin":
                    ...
                    depth -= 1
        if depth != 0:
            problems.append("%s p[%d]: unbalanced field (begin/end mismatch)"
                            % (part, index))
...
def check_bookmarks(root, part, problems):
    ...
    for bname, n in counts.items():
        if n > 1:
            problems.append("%s: bookmark name %r defined %d times"
                            % (part, bname, n))

    for link in root.iter(q("hyperlink")):
        anchor = link.get(q("anchor"))
        if anchor and anchor not in names:
            problems.append("%s: hyperlink anchor %r has no bookmark"
                            % (part, anchor))
    ...
    for ref in refs:
        if ref not in names:
            problems.append("%s: PAGEREF target %r has no bookmark"
                            % (part, ref))
    return names, refs
...
def check_numbering(root, problems):
    ...
    for child in root:
        ...
        if name == "num":
            ...
            problems.append(
                "numbering.xml: <w:abstractNum> after <w:num>; all abstract "
                "definitions must come first")

    abstract_ids = set()
    for definition in root.findall(q("abstractNum")):
        abstract_ids.add(definition.get(q("abstractNumId")))
        check_order(definition, "abstractNum",
                    "abstractNum[%s]" % definition.get(q("abstractNumId")),
                    problems)
        for lvl in definition.findall(q("lvl")):
            check_order(lvl, "lvl", "abstractNum[%s]/lvl[%s]"
                        % (definition.get(q("abstractNumId")),
                           lvl.get(q("ilvl"))), problems)

    for instance in root.findall(q("num")):
        check_order(instance, "num", "num[%s]" % instance.get(q("numId")),
                    problems)
        target = instance.find(q("abstractNumId"))
        if target is not None and target.get(q("val")) not in abstract_ids:
            problems.append("numbering.xml: num %s points at missing "
                            "abstractNum %s" % (instance.get(q("numId")),
                                                target.get(q("val"))))
...
def check_num_refs(doc_root, numbering_root, problems):
    ...
    for numpr in doc_root.iter(q("numPr")):
        ...
        if num_id is not None and num_id.get(q("val")) not in defined:
            problems.append("document.xml: numPr references undefined numId %s"
                            % num_id.get(q("val")))
...
def validate(path):
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        summary = {
            "bookmarks": len(marks),
            "pagerefs": len(refs),
            "update_on_open": settings is not None
            and settings.find(q("updateFields")) is not None,
        }
...
def main(argv=None):
    ...
    print("  bookmarks: %d, PAGEREF targets: %d, update-fields-on-open: %s"
          % (summary["bookmarks"], summary["pagerefs"],
             summary["update_on_open"]))
    if not problems:
