...
def build_where(schema, conditions):
    ...
    def bind(value):
        """Register a value, return its :pN placeholder string."""
        key = f"p{len(params)}"
        params[key] = value
        return f":{key}"
    ...
    for c in conditions:
        ...
        if ftype == "tsvector":
            fragments.append(f'"{schema["tsvector_col"]}" @@ plainto_tsquery({bind(val)})')
            ...
            if op == "is_empty":
                ...
                fragments.append(f"{col} @> {bind(val)}")
            elif op == "contains_any":
                fragments.append(f"{col} && {bind(val)}")
            elif op == "contains_none":
                fragments.append(f"NOT ({col} && {bind(val)})")
            elif op == "eq":
                fragments.append(f"{col} = {bind(val)}")

        elif op in ("in", "not_in"):
            placeholders = ", ".join(bind(v) for v in val)
            keyword = "IN" if op == "in" else "NOT IN"
            fragments.append(f"{col} {keyword} ({placeholders})")

        else:
            fragments.append(f"{col} {SCALAR_OPS[op]} {bind(val)}")
...
def build_sql(schema, conditions, cols, limit, offset):
    ...
    Both SQL strings use named :pN placeholders plus :_limit / :_offset.
    Both param dicts are ready to pass to SQLAlchemy Connection.execute().
    """
    ...
    data_params = {**where_params, "_limit": limit, "_offset": offset}
...
def get_db():
    """
    Wire this up to your engine, e.g.:
