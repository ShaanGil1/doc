...
def set_run_font(run, font_name: str, size_pt: Optional[float] = None):
    """Point all four character ranges at one font. Setting only .name leaves
    Word free to use the theme font for the ranges it did not cover"""
...
def set_paragraph_mark_font(paragraph, font_name: str, size_pt: float,
                            color: str = "000000", bold: bool = False):
    """Style the paragraph MARK rather than its text.

    Word draws a list's auto-number from the paragraph mark's run properties,
    so a Heading style's blue survives on the "1." even when every visible run
    is black. This is the only way to reach it"""
...
def ensure_style(document, style_name: str, based_on: str = "Normal"):
    ...
    Word ships "TOC Heading" but not "TOC 1"/"TOC 2"/"TOC 3", so a generated
    table of contents has nothing to hang formatting on until these exist"""
    try:
...
#
# What a cross referencing document needs (a table of contents, page numbers
# that follow the content) and python-docx has no API for. All generic OOXML
# =========================================================================== #
...
def add_bookmark(paragraph, name: str):
    ...
    <w:bookmarkStart> must come AFTER <w:pPr>, because the schema wants pPr as
    the first child of a paragraph. Word silently drops a bookmark that breaks
    this, which kills every reference to it. Other viewers do not care, so it
...
def add_field(paragraph, instruction: str, placeholder: str = "1",
    ...
    Each part gets its OWN run. Packing begin/instruction/separate/result/end
    into a single run is schema legal but Word will not evaluate it, so the
    field renders blank. The placeholder is what shows until Word recalculates.
    Returns the runs so the caller can style them.

    dirty marks the cached result as stale. Without it Word is entitled to
    keep showing the placeholder, which is why every page number in a
    generated table of contents comes out as "1" until someone presses F9"""
    field_parts = (
        ("fldChar", "begin"),
...
def wrap_paragraph_in_internal_link(paragraph, bookmark_name: str):
    ...
    An internal link uses w:anchor rather than an r:id relationship, so unlike
    an external hyperlink there is nothing to register in the rels part"""
...
def update_fields_on_open(document):
    ...
    Without this a PAGEREF shows its stale cached value until someone presses
    F9. <w:updateFields> has a fixed slot in the settings sequence, so it
...
# (numFmt, marker shape) pairs. "{}" is where Word's level counter token goes,
