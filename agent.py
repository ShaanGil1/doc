"""The section agent: SectionAgent(provider).find_boundaries(markdown) -> DlaiDocument, on the backend's LLM class.
...
for extra_path in (HERE, HERE.parent / "md_to_docx", HERE.parent):
    if str(extra_path) not in sys.path:
        sys.path.insert(0, str(extra_path))

import boundaries  # noqa: E402  md_to_docx
import config as cfg  # noqa: E402  md_to_docx/config.py
from models.boundary_map import COVER_KEYS, FIXED_KINDS, SECTION_FIELDS, BoundaryMap, Where, field_name  # noqa: E402
from models.prompts import EXAMPLE_ANSWER, EXAMPLE_DOC, INSTRUCTION, RECONCILE, RETRY  # noqa: E402
...
from services.v2.llm import LLM  # noqa: E402  the backend's model factory

# behaviour knobs
ATTEMPTS = 3  # model calls per question, for transient errors (429, 503, timeouts)
BACKOFF_SECONDS = 2.0  # wait before the second attempt; doubles each time
RETRIES = 1  # extra questions for blocks that failed validation
SEARCH_WINDOW = 5  # lines either side of a reported number to look for the quote
FALLBACK_TO_REGEX = True  # model unreachable -> the regex provider, with a finding
RECONCILE_WITH_RULES = True  # model and rules disagree on a line -> one question to pick
BACKFILL_FROM_RULES = True  # blanks and stubborn failures -> the rules, with a finding
FATAL = ("credentials", "api_key", "401", "403", "Unsupported LLM provider", "DeploymentNotFound")  # no point retrying
...
class SectionAgent:
    """Finds where each block starts. `model` is any LangChain chat model; by default the backend's LLM(provider)."""

    def __init__(self, provider: str = "azure", model=None):
        self.provider = provider
        self.model = model
        self.last: dict = {}  # {"attempts", "seconds"} of the last successful call

    def chat_model(self):
        if self.model is None:
            self.model = LLM(provider=self.provider).langchain_model
        return self.model

    def structured(self, instruction: str, prompt: str, schema):
        """Ask for an instance of `schema` via with_structured_output; ATTEMPTS tries with backoff; raises LlmUnavailable."""
        messages = [SystemMessage(content=instruction), HumanMessage(content=prompt)]
        wait, failures = BACKOFF_SECONDS, []
        for attempt in range(1, ATTEMPTS + 1):
            started = time.time()
            try:
                result = self.chat_model().with_structured_output(schema).invoke(messages)
                if isinstance(result, dict):
                    result = schema.model_validate(result)
                if result is None:
                    raise ValueError("model returned no structured answer")
                self.last = {"attempts": attempt, "seconds": round(time.time() - started, 1)}
                return result
            except Exception as error:  # transient (429, 503, timeout): retry; credentials or auth: stop at once
                failures.append("%s: %s" % (type(error).__name__, str(error)[:160]))
                if any(word in str(error) for word in FATAL):
                    break
                if attempt < ATTEMPTS:
                    time.sleep(wait)
                    wait *= 2
    ...
    def find_boundaries(self, markdown_text: str) -> DlaiDocument:
        """Markdown in, DlaiDocument out; hand the result to template_processor.md_to_docx."""
        lines = boundaries.preprocess(markdown_text)
        starts, mode, findings = self.llm_boundaries(lines)
        doc = boundaries.assemble(lines, starts, mode)
        doc.findings[0:0] = findings
        return doc
    ...
    def retry_call(self, result, placed, failed, reasons, lines, prompt, findings):
...
def title_shaped(kind: str, line: str) -> bool:
    ...
    if HEADING_LINE.match(text):
        return True
    if kind == "signature" and (FENCE_LINE.match(text) or SIGNATURE_LINE.match(text)):
        return True
    if kind == "cover":
...
# module-level shortcuts, for a main that wants the two providers side by side
def llm_boundaries(lines: List[str], provider: str = "azure") -> Tuple[List[Start], str, List[str]]:
    return SectionAgent(provider).llm_boundaries(lines)


def find_boundaries(markdown_text: str, provider: str = "azure") -> DlaiDocument:
    return SectionAgent(provider).find_boundaries(markdown_text)
