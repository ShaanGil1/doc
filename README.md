...
from models import Citation, ExtractedClaim, RetrievedChunk, ReviewerViolation, Suggestion
...
class ConflictFinding(BaseModel):
    ...
    source_chunk_index: int = Field(description="1-based index of the source that triggered the conflict (matches the numbered SOURCES list)")
...
def find_conflict(
    ...
    for finding in response.conflicts:
        ...
        # chunks is the section's deduped pool; the LLM saw it numbered and the
        # index maps straight back into the same list, so the consolidation is
        # safe (this is what the old "break on merge" TODO was worried about).
        ...
        citation = Citation(
            ...
            document_name=source_chunk.doc_title,
            text=finding.source_excerpt,
        )
        ...
        violations.append(ReviewerViolation(
            source_agent="reviewer",
            violation_category="source_conflict",
            violation_citation=[citation],
            violation_description=f"{finding.reason}",
